...
class BP000EnvGenesis(RLEnvGenesis):
    ...
    def specific_update_buffer(self):
        # HRP2の足リンク名 (RLEG_LINK5, LLEG_LINK5) に変更
        self.l_ankle_z[:] = get_link_lowest_point_z(self.robot.get_link(name="LLEG_LINK5"))
        self.r_ankle_z[:] = get_link_lowest_point_z(self.robot.get_link(name="RLEG_LINK5"))
...
##追加リワード
    def _reward_ankle_regularization(self):
        ...
        """
        # HRP2用デフォルトリンク名
        foot_names = self.env_cfg.get("feet_link_names", ["LLEG_LINK5", "RLEG_LINK5"])
    ...
    def _reward_hip_pitch_motion(self):
        ...
        if not hasattr(self, "_hip_pitch_indices"):
            ...
            # ★HRP2のHip Pitch関節名 (RLEG_JOINT2, LLEG_JOINT2)
            target_joints = ["RLEG_JOINT2", "LLEG_JOINT2"]
    ...
    def _reward_knee_pitch_motion(self):
        ...
        if not hasattr(self, "_knee_pitch_indices"):
            ...
            # ★HRP2のKnee Pitch関節名 (RLEG_JOINT3, LLEG_JOINT3)
            target_joints = ["RLEG_JOINT3", "LLEG_JOINT3"]
            ...
            self._knee_pitch_indices = torch.tensor(indices, device=self.device, dtype=torch.long)
            
        if len(self._knee_pitch_indices) == 0:
            return 0.0
            
        target_angles = self.dof_pos[:, self._knee_pitch_indices]
        return torch.mean(torch.square(target_angles), dim=1)
