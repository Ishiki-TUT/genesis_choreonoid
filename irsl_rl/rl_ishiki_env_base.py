...
class RLEnvBase:
    ...
    def _reward_tracking_lin_vel(self):
        ...
        return torch.exp(-lin_vel_error / self.reward_cfg["tracking_sigma"]) # -lin_vel_error / 0.25 これ要素の数じゃない?

    def _reward_tracking_ang_vel(self):
        # Tracking of angular velocity commands (yaw)
        ang_vel_error = torch.square(self.commands[:, 2] - self.base_ang_vel[:, 2])
        return torch.exp(-ang_vel_error / self.reward_cfg["tracking_sigma"]) # -ang_vel_error / 0.25
